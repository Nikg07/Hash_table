...
void linear_adler_init(HashTableLinearAdler* t, int capacity, float load_factor) {
    ...
    t->collision_count = 0;
    t->resize_count = 0;
    t->entries = (EntryLinearAdler*)calloc(capacity, sizeof(EntryLinearAdler));
}

static void linear_adler_resize(HashTableLinearAdler* t) {
    int old_cap = t->capacity;
    EntryLinearAdler* old = t->entries;
    int new_cap = old_cap * 2;
    t->entries = (EntryLinearAdler*)calloc(new_cap, sizeof(EntryLinearAdler));
    t->capacity = new_cap;
    t->size = 0;
    ...
    for (int i = 0; i < old_cap; i++) {
        if (old[i].state == OCCUPIED) {
            ...
            linear_adler_put(t, old[i].key, buf);
            free(old[i].key);
        }
...
int linear_adler_put(HashTableLinearAdler* t, const char* key, const char* val_str) {
    unsigned int hash = adler32(key, (unsigned int)strlen(key));
...
void linear_adler_list(const HashTableLinearAdler* t, int num) {
    printf("=== Hash Table (linear search, adler32) ===\n");
